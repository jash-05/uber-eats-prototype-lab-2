...
exports.findOneRestaurant = async (args) => {
	...
	try {
		...
		console.error(err);
		return { error: "Error while fetching data for restaurant: ", err };
	}
};

exports.getDishesbyRest = async (args) => {
	const restaurant_ID = args.rest_ID;
    try {
        const response = await Restaurant.find({_id: restaurant_ID, "dishes"})
        return response[0].dishes;
    } catch (err) {
        console.error(err);
        return {error: "Error occured while trying to fetch dishes: ", err}
    }
};
